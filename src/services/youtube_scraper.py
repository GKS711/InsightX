"""
InsightX YouTube 留言爬蟲服務 v1.2 — v6 sync edition
...
v6 sync 轉換：
  - 所有 async def → def
  - 移除 asyncio.to_thread 包 sync 函式（HTTP 已經是 sync）
"""
...
class YouTubeScraper:
    ...
    def scrape_video(self, url: str, max_comments: int = 200) -> dict:
        """
        ...
        if self._fallback_mode == "force-ytdlp":
            ...
            return self._scrape_via_ytdlp(url, video_id, max_comments)
        ...
        print(f"{'='*55}")
        result = self._scrape_via_official_api(url, video_id, max_comments)

        # 官方失敗 且 允許 fallback
        if result["status"] == "error" and self._fallback_mode != "off":
            print(f"\n[YT] 官方 API 失敗（{result.get('error')}），自動切換到 yt-dlp 備用模式")
            fallback_result = self._scrape_via_ytdlp(url, video_id, max_comments)
            if fallback_result["status"] != "error":
                return fallback_result
            # 兩個都失敗：合併兩個錯誤訊息（避免「備用沒裝 / 備用 import 失敗」被 main API error 蓋掉）
    ...
    def _scrape_via_official_api(self, url: str, video_id: str, max_comments: int) -> dict:
    ...
    def _fetch_video_info(self, video_id: str) -> dict:
        ...
        data = self._yt_api_get("videos", params)
    ...
    def _fetch_comments(self, video_id: str, max_comments: int) -> list:
        ...
        while len(comments) < max_comments:
            ...
            data = self._yt_api_get("commentThreads", params)
            if not data:
                break
    ...
    def _yt_api_get(self, endpoint: str, params: dict) -> dict:
        """v6: sync HTTP call (was wrapped in asyncio.to_thread before)."""
    ...
    def _scrape_via_ytdlp(self, url: str, video_id: str, max_comments: int) -> dict:
        """
        ...
        video_info = self._fetch_meta_via_oembed(video_id)
        ...
        try:
            ...
            raw_comments = list(itertools.islice(gen, max_comments))
        except Exception as e:
            return self._err(
                url,
